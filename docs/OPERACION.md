...
4. Migraciones. Primero ver cuáles registra la base:
   ...
   - Si aparecen las migraciones anteriores, aplicar las nuevas con `pnpm exec prisma migrate deploy` usando el `DATABASE_URL` de producción.
   - Si la tabla `_prisma_migrations` no existe, la base se creó sin Prisma Migrate: no correr `migrate deploy` a ciegas. La migración 031 es idempotente y se puede aplicar directo con `psql -f`, y después marcar las migraciones como aplicadas con `prisma migrate resolve --applied <nombre>`.
