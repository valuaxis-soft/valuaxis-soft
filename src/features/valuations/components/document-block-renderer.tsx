...
import type { Block, Concept, ContentLayoutPersisted, ImageContent, Apartado, TableContent, ContentLayoutRowV2, ContentLayoutItemRef } from "@/features/valuations/model";
...
type DocumentBlockRendererProps = {
  ...
  /** Optional: render block title bar (default: true for generic) */
  renderTitleBar?: boolean;
  /** Optional: extra CSS class for the block container */
  className?: string;
  /** Optional: custom title renderer (replaces DocumentBlockTitleBar) */
  renderBlockTitle?: (block: Block) => React.ReactNode;
  /** Optional: custom content wrapper for content rows */
    ...
    <section className={className}>
      ...
      {structuralRows.map((structuralRow) => {
        ...
        if (firstItem.type === "apartado" && structuralRow.items.length === 1) {
          ...
          if (!subBlock || subBlock.enabled === false) return null;
          return (
            <DocumentApartado
              key={`ap-${structuralRow.id}`}
              block={block}
              subBlock={subBlock}
        ...
        if (firstItem.type === "apartado" && structuralRow.items.length === 2) {
          ...
          return (
            <div key={`pair-${structuralRow.id}`} className={`mt-1 grid grid-cols-2 gap-4 ${pairedApartadosClassName ?? ""}`}>
